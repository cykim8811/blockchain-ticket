...
import { Loader2, Armchair, BellRing, Info, CheckCircle } from "lucide-react";
import { useAuth } from "@/contexts/AuthContext";

// Mock seat data generator - Now starts all available
const generateSeats = (_count: number) => {
...
export default function TicketingView() {
  ...
  const [processingStatus, setProcessingStatus] = useState<
    "idle" | "sending" | "confirming" | "booking"
  >("idle");
  const [isLoaded, setIsLoaded] = useState(false);
  const [seats, setSeats] = useState(generateSeats(totalSeats));
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, async (snapshot) => {
      ...
      const bookedDocs = snapshot.docs.filter(
        (doc) => doc.data().status === "booked"
      );
      ...
      const bookedSeatNumbers = new Set(
        bookedDocs.map((doc) => doc.data().seatNumber)
      );
      ...
      if (user) {
        ...
        // Check if user already has a booked ticket
        const myBookedTicket = bookedDocs.find(
          (doc) => doc.data().userId === user.uid
        );
        setHasBookedTicket(!!myBookedTicket);

        // Find user's waiting ticket
        const myWaitingTicket = snapshot.docs.find(
          (doc) =>
            doc.data().userId === user.uid && doc.data().status === "waiting"
        );

        if (myWaitingTicket) {
          // Calculate rank
          const myIndex = waitingDocs.findIndex(
            (doc) => doc.id === myWaitingTicket.id
          );
          ...
          if (waitingDocs.length > 0) {
            ...
            // No waiters. Eligible ONLY if there are seats available.
            setIsEligible(availableSeats > 0);
          }
        }
      }
    });
  ...
  const getStepTitle = () => {
    switch (step) {
      ...
      case 2:
        return "Select Seat";
      case 3:
        return isFull ? "Confirm Waitlist" : "Confirm Booking";
      default:
        return "";
    }
  };
  ...
  const getStepDescription = () => {
    switch (step) {
      ...
      case 2:
        return "Choose your preferred seat";
      case 3:
        return isFull
  ...
  const getButtonText = () => {
    switch (processingStatus) {
      case "sending":
        return "Sending Transaction to ETC...";
      case "confirming":
        return "Confirming Transaction...";
      case "booking":
        return isFull ? "Joining Waitlist..." : "Finalizing Booking...";
      default:
        return isFull ? "Join Waitlist" : "Confirm & Pay";
    }
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 md:p-8 relative overflow-hidden">
      {/* Blurred Background */}
      <div
        className={`absolute inset - 0 z - [-1] bg - cover bg - center transition - all duration - 1000 ease - out ${
        ...
        <CardContent>
          ...
          <div className="mb-6 rounded-lg border bg-muted/50 p-4">
            <div className="flex flex-col space-y-2">
              ...
              {hasBookedTicket && (
                <div className="mt-4 p-3 bg-primary/10 border border-primary/20 text-primary rounded-md text-sm font-medium flex items-center">
              ...
              {!hasBookedTicket && waitlistPosition !== null && (
                <div
                  className={`mt-4 p-3 rounded-md text-sm font-medium flex items-center animate-pulse ${
                    isEligible
                ...
                >
                  {isEligible ? (
                    <>
                      ...
                      You are #{waitlistPosition} in line! It's your turn to
                      book.
                    </>
                  ) : (
                    <>
              ...
              {!hasBookedTicket &&
                ...
                bookedCount >= totalSeats && (
                  <div className="mt-4 p-3 bg-green-100 border border-green-200 text-green-800 rounded-md text-sm font-medium flex items-center animate-pulse">
                    <BellRing className="w-4 h-4 mr-2" />A spot is open! You can
                    now select a seat.
                  </div>
                )}
          ...
          {step === 1 && (
            <form onSubmit={handleNext} className="space-y-4">
              ...
              <div className="pt-4 grid grid-cols-2 gap-4">
                ...
                <Button type="submit" disabled={hasBookedTicket}>
                  {hasBookedTicket
                    ? "Already Booked"
                    : isFull
                    ? "Next: Join Waitlist"
            ...
            <div className="space-y-6">
              <div className="rounded-lg border p-4 space-y-4">
                <h3 className="font-semibold">
                  {isFull ? "Waitlist Details" : "Booking Details"}
                </h3>
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <span
                      className={`font - medium ${
                        isFull ? "text-yellow-600" : "text-green-600"
                      } `}
                    >
                ...
                {isFull && (
                  <div className="bg-yellow-50 dark:bg-yellow-900/20 p-3 rounded text-sm text-yellow-800 dark:text-yellow-200">
                    The event is currently full. You will be added to the
                    waitlist.
                  </div>
                )}
              </div>
